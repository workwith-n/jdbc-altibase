...
ResultSet.prototype.toObjArray = async function (callback) {
  await this.toObject(function (err, result) {
    if (err) return callback(err);
    callback(null, result.rows);
  });
};

ResultSet.prototype.toObject = async function (callback) {
  await this.toObjectIter(function (err, rs) {
    if (err) return callback(err);
...
ResultSet.prototype.toObjectIter = async function (callback) {
  var self = this;

  await self.getMetaData(async function (err, rsmd) {
    if (err) {
      return callback(err);
    } else {
      var colsmetadata = [];

      await rsmd.getColumnCount(function (err, colcount) {
