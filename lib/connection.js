...
Pool.prototype.reserve = function (callback) {
  ...
  // 변경
  console.log('pool.js pool(P): %i / %i', self._reserved.length, self._pool.length)
  ...
  // 변경
  console.log('pool.js pool(R): %i / %i', self._reserved.length, self._pool.length)
...
function closeIdleConnectionsInArray(array, maxIdle) {
  ...
  for (var i = array.length - 1; i >= 0; i--) {
    ...
    if (typeof conn === 'object' && conn.conn !== null) {
      console.log('conn.lastIdle: $i($i)', conn.lastIdle, maxLastIdle);
