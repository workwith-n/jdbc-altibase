...
Statement.prototype.executeUpdate = async function(sql, arg1, callback) {
  ...
  await this._s.executeUpdate.apply(this._s, args);
};

Statement.prototype.executeQuery = async function(sql, callback) {
  if (typeof sql === 'string') {
    await this._s.executeQuery(sql, async function(err, resultset) {
      if (err) {
        return callback(err);
      } else {
        return callback(null, await new ResultSet(resultset));
      }
    });
  } else {
    return callback(new Error('INVALID ARGUMENTS'));
