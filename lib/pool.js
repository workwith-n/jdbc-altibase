...
Pool.prototype.reserve = async function (callback) {
  var self = this;
  var conn = null;
...
Pool.prototype._closeIdleConnections = async function () {
  if (!this._maxidle) {
    return;
  }
  ...
  await closeIdleConnectionsInArray(self._pool, this._maxidle);

  // 변경
  // closeIdleConnectionsInArray(self._reserved, this._maxidle);
};

async function closeIdleConnectionsInArray(array, maxIdle) {
  ...
  for (var i = array.length - 1; i >= 0; i--) {
    ...
    if (typeof conn === 'object' && conn.conn !== null) {
      ...
      if (conn.lastIdle < maxLastIdle) {
        await conn.conn.close(function (err) {
          if(err) {
...
Pool.prototype.release = async function (conn, callback) {
  var self = this;
