...
Connection.prototype.close = async function(callback) {
  var self = this;
  ...
  await self._conn.close(function(err) {
    if (err) {
      return callback(err);
    } else {
      self._conn = null;
...
Connection.prototype.createStatement = async function(arg1, arg2, arg3, callback) {
  ...
  args.push(async function (err, statement) {
    if (err) {
      return callback(err);
    } else {
      return callback(null, await new Statement(statement));
    }
  });
